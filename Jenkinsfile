...
environment {
    ...
    // Replace with your ECR repository URI (e.g., 123456789012.dkr.ecr.us-east-1.amazonaws.com/my-nginx-repo)
    ...
    stage('Push to ECR') {
        steps {
            script {
                ...
                // Assumes AWS credentials are configured in Jenkins and available to the agent
                echo "Authenticating with ECR in region: ${AWS_REGION}"
                // Use the AWS CLI to get the Docker login command and execute it
